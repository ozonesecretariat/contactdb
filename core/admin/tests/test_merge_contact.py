...
class TestMergeContacts(TestCase):
    ...
    def setUp(self):
        ...
        self.registered = RegistrationStatus.objects.get(name="Registered")
        self.accredited = RegistrationStatus.objects.get(name="Accredited")
    ...
    def test_merge_duplicate_contacts(self):
        ...
        """
        contact1_dict = Contact.objects.filter(id=self.contact1.id).values()[0]
        contact1_dict.pop("id", None)

        duplicate_contact = Contact.objects.create(**contact1_dict)
        duplicate_contact.birth_date = datetime(1992, 1, 1, tzinfo=UTC)
